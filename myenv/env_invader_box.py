...
class InvaderBoxEnv(gym.Env):
    ...
    def step(self, invader_action):
        ...
        if np.random.rand() < self.draw_probability:
            ...
            chaser_action = self.chaser_func(obs)
            new_chaser_position = np.array(self.chaser_position) + np.array(
                switcher.get(chaser_action))
    ...
    def state_to_observation(self):
        xA, yA = self.chaser_position
        xB, yB = self.invader_position
        x_max = max(abs(self.board_right), abs(self.board_left))
        y_max = max(abs(self.board_top), abs(self.board_bottom))
        obs = np.array(
            [[xA / x_max,
              yA / y_max,
              xB / x_max,
