...
class DqnSelfplay:
    ...
    def replay_buffer_sample(self, batch_size):
        ...
        for sample in samples:
            obses.append(sample[0])
            acts.append(sample[1])
            rews.append(sample[2])
            obses_tp1.append(sample[3])
            dones.append(sample[4])
        return obses, acts, rews, obses_tp1, dones
    ...
    def _train_step(self, obses_t, actions, rewards, obses_tp1, dones):
        if self.policy == 'tabular':
            ...
            q_tp1_best = (1.0 - np.array(dones)) * q_tp1_best
            ...
            q_t_selected_target = rewards + self.gamma * q_tp1_best
            td_error = q_t[range(len(obses_t)),actions] - q_t_selected_target

            q_t[range(len(obses_t)), actions] = q_t_selected_target
    ...
    def learn(self, total_timesteps, train_save_freq=None, train_save_log_name='dqn_selfplay',
        ...
        for _ in range(total_timesteps):
            ...
            mirror_obs = self.env.mirror_observation()
            bct = self.act(mirror_obs, update_eps)
            bct = self.env.mirror_action(bct)
