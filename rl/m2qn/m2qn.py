...
import os
import json
from rl.common.rl_util import minimax, get_linear_epsilon_value
...
class M2qn:
    ...
    def __init__(self, policy, env, gamma=0.99, learning_rate=5e-4, buffer_size=50000, exploration_fraction=0.1,
                 ...
                 learning_starts=1000, evaluated_network_update_freq=500, target_network_update_freq=500,
        ...
        self.evaluated_network_update_freq = evaluated_network_update_freq
        ...
        if self.policy == 'tabular':
            ...
            self.target_values = np.zeros([s_dim, a_dim, b_dim])
            self.evaluated_values = np.zeros([s_dim, a_dim, b_dim])
            if self.double_q:
                self.double_values = self.q_values
        elif policy == 'mlp':
            s_dim = self.env.observation_space.shape
            self.q_values = self._build_policy(
                s_dim, a_dim, b_dim, self.policy_kwargs['layer_size'])
            self.target_values = self._build_policy(
                s_dim, a_dim, b_dim, self.policy_kwargs['layer_size'])
            self.evaluated_values = self._build_policy(
                s_dim, a_dim, b_dim, self.policy_kwargs['layer_size'])
            if self.double_q:
                self.double_values = self.q_values
    ...
    def _build_policy(self, input_shape, num_actionsA, num_actionsB, layer_size):
        inputs = Input(shape=input_shape)
        action_out = Dense(layer_size[0], activation='relu')(inputs)
        for i in range(1, len(layer_size)):
            action_out = Dense(layer_size[i], activation='relu')(action_out)
        action_scores = Dense(num_actionsA*num_actionsB)(action_out)
        ...
        if self.dueling:
            ...
            state_score = Lambda(lambda state_score: tf.tile(
                state_score, [1, num_actionsA*num_actionsB]))(state_score)
            q_out = Add()([state_score, action_scores_centered])
        else:
            q_out = action_scores

        q_out = Reshape((num_actionsA, num_actionsB))(q_out)
    ...
    def replay_buffer_add(self, obs, act, bct, rew, n_obs, done):
        self.replay_buffer.append((obs, act, bct, rew, n_obs, done))

    def replay_buffer_sample(self, batch_size):
        samples = random.sample(self.replay_buffer, batch_size)
        obses, acts, bcts, rews, obses_tp1, dones = [], [], [], [], [], []
        for sample in samples:
            obses.append(np.squeeze(sample[0]))
            acts.append(sample[1])
            bcts.append(sample[2])
            rews.append(sample[3])
            obses_tp1.append(np.squeeze(sample[4]))
            dones.append(sample[5])
        return (np.array(obses), np.array(acts), np.array(bcts),
                np.array(rews), np.array(obses_tp1), np.array(dones))


    def _train_step(self, obses_t, actions, bctions, rewards, obses_tp1, dones):
        if self.policy == 'tabular':
            q_t_selected = self.q_values[obses_t,actions,bctions]
            q_tp1_target = self.target_values[obses_tp1,:,:]            # (batch, nA, nB)
            q_tp1_evaluated = self.evaluated_values[obses_tp1,:,:]      # (batch, nA, nB)
            piA_tp1 = np.zeros([self.batch_size, self.env.action_space.n])    # (batch, nA)
            for i in range(self.batch_size):
                piA_tp1[i,:] = minimax(q_tp1_evaluated[i,:,:])
            if self.double_q:
                q_tp1_double = self.double_values[obses_tp1,:,:]        # (batch, nA, nB)
                q_tp1_best = np.sum(q_tp1_double * np.expand_dims(
                    piA_tp1, axis=2), axis=1, keepdims=False)       # (batch, nB)
                bct_tp1 = np.argmin(q_tp1_best, axis=-1)                # (batch, )
                bct_tp1_onehot = np.zeros((self.batch_size, self.env.bction_space.n))
                bct_tp1_onehot[range(self.batch_size), bct_tp1] = 1      # (batch, nB)
                q_tp1_worst = np.sum(q_tp1_target * np.expand_dims(
                    bct_tp1_onehot, axis=1), axis=2)     # (batch, nA)
                q_tp1_best_worst = np.sum(piA_tp1 * q_tp1_worst, axis=1, keepdims=False)      # (batch, )
            else:
                q_tp1_best_worst = np.amin(np.sum(
                    q_tp1_target*piA_tp1, axis=1, keepdims=False),
                    axis=1)         # (batch, )
            q_tp1_best_worst = (1.0 - dones) * q_tp1_best_worst

            q_t_selected_target = rewards + self.gamma * q_tp1_best_worst
            td_error = q_t_selected_target - q_t_selected

            self.q_values[obses_t,actions,bctions] += self.learning_rate * td_error
            return np.sum(td_error), None

        elif self.policy == 'mlp':
            q_t = self.q_values.predict(obses_t)       # (batch, nA, nB)
            q_tp1_target = self.target_values.predict(obses_tp1)    # (batch, nA, nB)
            q_tp1_evaluated = self.evaluated_values.predict(obses_tp1)      # (batch, nA, nB)
            piA_tp1 = np.zeros([self.batch_size, self.env.action_space.n])        # (batch, nA)
            for i in range(self.batch_size):
                piA_tp1[i, :] = minimax(q_tp1_evaluated[i, :, :])
            if self.double_q:
                ...
                q_tp1_best_worst = np.argmin(np.sum(
                    q_tp1_target * piA_tp1, axis=1, keepdims=False),
                    axis=1)  # (batch, )
            q_tp1_best_worst = (1.0 - dones) * q_tp1_best_worst

            q_t_selected_target = rewards + self.gamma * q_tp1_best_worst
            td_error = q_t[range(len(obses_t)),actions,bctions] - q_t_selected_target

            q_t[range(self.batch_size), actions, bctions] = q_t_selected_target
    ...
    def bct(self, obs, update_eps=0.0):
        if np.random.rand() < update_eps:
            return np.random.choice(range(self.env.bction_space.n))
        else:
            if self.policy == 'tabular':
                Q = self.q_values[obs, :, :]
            elif self.policy == 'mlp':
                Q = self.q_values.predict(obs)[0]    # (nA,nB)
            else:
                raise NotImplementedError
            prob_best = minimax(Q)        # (nA,)
            q_best = np.matmul(prob_best, Q)   # (nB,)
            return np.argmin(q_best)


    def learn(self, total_timesteps, train_save_freq=None, train_save_log_name='m2qn',
        ...
        for _ in range(total_timesteps):
            ...
            bct = self.bct(obs, update_eps)
            new_obs, rew, done, info = self.env.step(act, bct)
            num_timesteps += 1

            self.replay_buffer_add(obs, act, bct, rew, new_obs, done)

            if len(self.replay_buffer) > self.batch_size \
                and num_timesteps > self.learning_starts:
                experience = self.replay_buffer_sample(self.batch_size)
                obses_t, actions, bctions, rewards, obses_tp1, dones = experience
                self._train_step(obses_t, actions, bctions, rewards, obses_tp1, dones)
                ...
                if num_timesteps % self.evaluated_network_update_freq == 0:
                    if self.policy == 'tabular':
                        self.evaluated_values = np.copy(self.q_values)
                    elif self.policy == 'mlp':
                        self.evaluated_values.set_weights(self.q_values.get_weights())
    ...
    def save(self, save_path):
        params = {
            ...
            "evaluated_network_update_freq": self.evaluated_network_update_freq,
