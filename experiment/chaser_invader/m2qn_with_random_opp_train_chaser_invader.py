...
def invader_random_func(obs):
    return np.random.choice(range(chaser_invader_discrete_env.bction_space.n))
...
m2qn_random_agent = M2qnOpp(
    ...
    bct_func=invader_random_func,
    policy_kwargs={'layer_size': [50, 50]},
    seed=300)


m2qn_random_agent.learn(
    total_timesteps=40000,
    train_save_freq=500,
    train_save_log_name='m2qn_with_random_opp_chaser_invader',
    train_evaluate_freq=500,
    train_evaluate_func=lambda act_func: evaluate_chaser_invader(
        ChaserInvaderBoxEnv(), act_func, dp_bct_func),
